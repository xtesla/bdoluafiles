...
function Panel_Tooltip_Item_ShowInfo(target, inputValue, isSSW, isItemWrapper, chattingLinkedItem, index, isNextEnchantInfo, invenSlotNo, itemNamingStr)
  ...
  elseif enchantLevel > 0 and CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and false == itemSSW:isSpecialEnchantItem() then
    ...
    local itemMakerStringKey
    if 1 == currentItemType then
      itemMakerStringKey = "LUA_TOOLTIP_ITEM_MAXENCHANTER"
    elseif 2 == currentItemType then
      itemMakerStringKey = "LUA_TOOLTIP_RESTOREITEM_NAMING"
    end
    if nil ~= itemNamingStr and nil ~= itemMakerStringKey then
      target.itemName:SetText(itemSSW:getName())
      target._txt_MaxEnchanter:SetShow(isMaxEnchanterEnable)
      target._txt_MaxEnchanter:SetText(PAGetStringParam1(Defines.StringSheet_GAME, itemMakerStringKey, "name", itemNamingStr))
    elseif nil ~= chattingLinkedItem and "" ~= chattingLinkedItem:getItemNamingUserName() then
      target.itemName:SetText(itemSSW:getName())
      target._txt_MaxEnchanter:SetShow(isMaxEnchanterEnable)
      target._txt_MaxEnchanter:SetText(PAGetStringParam1(Defines.StringSheet_GAME, itemMakerStringKey, "name", chattingLinkedItem:getItemNamingUserName()))
    else
      target.itemName:SetText(itemSSW:getName())
    end
  end
