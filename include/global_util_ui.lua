  ...
  if CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP == alignType then
    ...
    for name, value in pairs(reversedTable) do
      if true == value:GetShow() then
        local spaceFromRight = value:GetTextSizeX() + space
        local keySizeLT = 0
        if nil ~= prevControlName and (nil ~= string.find(prevControlName, "LT") or nil ~= string.find(prevControlName, "LB") or nil ~= string.find(prevControlName, "RT") or nil ~= string.find(prevControlName, "RB")) then
