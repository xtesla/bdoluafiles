...
function PaGlobal_WebControl:takeAndShow(helpType)
  ...
  elseif helpType == "PanelWindowStableShop" then
    if eCountryType.SA_REAL == serviceType or eCountryType.SA_ALPHA == serviceType then
  ...
  elseif helpType == "FamilyInventory" then
    local url = ToClient_getInGameFamilyInvenWikiURL()
    if nil == url or "" == url then
      url = PAGetString(Defines.StringSheet_GAME, "WEBHELPER_GAMEGUIDE")
    end
    PaGlobal_WebControl.web_control:SetUrl(self._webSizeX, self._webSizeY, url)
  elseif helpType == "HorseRace" then
    local url = ToClient_getInGameHorseRaceWikiURL()
    if nil == url or "" == url then
      url = PAGetString(Defines.StringSheet_GAME, "WEBHELPER_GAMEGUIDE")
    end
    PaGlobal_WebControl.web_control:SetUrl(self._webSizeX, self._webSizeY, url)
  ...
  elseif helpType == "SpecUpGuide" then
    ...
    if nil ~= url then
      local keyword = PaGlobal_WebControl:getWikiKeyword(helpType)
      PaGlobal_WebControl.web_control:SetUrl(self._webSizeX, self._webSizeY, url .. "/Wiki/Search/" .. tostring(keyword))
    else
      PaGlobal_WebControl.web_control:SetUrl(self._webSizeX, self._webSizeY, PAGetString(Defines.StringSheet_GAME, "WEBHELPER_GAMEGUIDE"))
    end
  else
