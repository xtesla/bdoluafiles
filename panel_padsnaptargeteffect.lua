...
function PadSnapTargetEffect:isAvailableUpdate()
  ...
  if nil == self._targetControl then
    ...
    if __ePadSnapAttrType_NoTargetEffect == self._targetControl:getPadSnapAttrType() then
      return false
    end
    local targetParentPanel = self._targetControl:GetParentPanel()
    if nil ~= targetParentPanel and self._targetControl:GetParentPosY() < targetParentPanel:GetParentPosY() then
      return false
    end
  end
  return true
...
function PaGlobalFunc_PadSnapTargetEffect_ChangeTarget(control)
  PadSnapTargetEffect:changeTarget(control)
end
