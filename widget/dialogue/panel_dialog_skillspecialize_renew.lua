...
function PaGlobalFunc_Dialog_SkillSpecialize_Doit()
  ...
  if nil == self._value.currnetReinforceIndex then
    ...
    local function doActual()
      ToClient_RequestSkillAwakening(__eSkillAwakeningActorType_NonPlayer, self._value.currentSelectSkillIndex, self._value.currentEffectIndex, self._value.currentEffectIndex2)
    end
  ...
  elseif nil == self._value.currentSelectSkillIndex then
    ...
    ToClient_RequestChangeAwakeningBitFlag(__eSkillAwakeningActorType_NonPlayer, self._value.currnetReinforceIndex, self._value.currentEffectIndex, self._value.currentEffectIndex2)
  else
    if nil == self._value.currentEffectIndex then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_SKILLREINFORCE_SELECTFIRSTOPTION"))
      return
    ...
    local function doActual()
      ToClient_RequestChangeAwakeningSkill(__eSkillAwakeningActorType_NonPlayer, self._value.currnetReinforceIndex, self._value.currentSelectSkillIndex, self._value.currentEffectIndex, self._value.currentEffectIndex2)
    end
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILLAWAKEN_AWAKENTITLE"),
      content = getEffectText(self._value.currentEffectIndex) .. "\n" .. getEffectText(self._value.currentEffectIndex2),
