...
function PaGlobal_NPCShop_ALL:siegeResetTimerRegister()
  ...
  self._curTime = 0
  self._siegeShopResetTime_64 = ToClient_getSiegeShopResetTime()
  local leftTime = getLeftSecond_TTime64(self._siegeShopResetTime_64)
  self._ui._txt_GuildLimitTime:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LIMITEDITEMSHOP_SIEGE") .. " " .. convertStringFromDatetimeType3(leftTime))
  Panel_Dialog_NPCShop_All:RegisterUpdateFunc("PaGlobalFunc_NPCShop_ALL_SiegeResetTimerUpdatePerFrame")
end
function PaGlobal_NPCShop_ALL:siegeResetTimerUpdatePerFrame(deltaTime)
  if nil == Panel_Dialog_NPCShop_All then
    return
  end
  self._curTime = self._curTime + deltaTime
  if self._curTime < 1 then
    return
  end
  self._curTime = 0
  local leftTime = getLeftSecond_TTime64(self._siegeShopResetTime_64)
