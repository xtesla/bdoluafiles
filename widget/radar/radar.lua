...
local chaseColor = UI_color.C_FFF5DA6F
...
function FromClient_MapSizeScale()
  ...
  if true == _ContentsGroup_HotDeal_All and nil ~= FromClient_Widget_HotDeal_All_OnResize then
    FromClient_Widget_HotDeal_All_OnResize()
  end
  if false == _ContentsGroup_RemasterUI_QuestWidget then
    PaGlobalFunc_Quest_UpdatePosition()
  end
...
function RadarMap_UpdateIcon()
  ...
  local function CheckSiegeArea(installType)
    ...
    local isRightSiegeTent = PaGlobalFunc_Radar_All_RightCheck(__eGuildRightType_Tent)
    ...
    for key2, actor in pairs(RadarIconActorList) do
      if true == actor:isPlayer() then
        ...
        if nil ~= playerActorProxyWrapper then
          ...
          local color = ToClient_GetObserverGuildColorByGuildNo(guildNoOrAllianceNo)
          if nil ~= color and nil ~= RadarIconList[key2] then
