...
function PaGlobalFunc_CharacterSelect_All_OnScreenResize()
  ...
  if true == PaGlobal_CharacterSelect_All._isUsePadSnapping then
    ...
    btnSize = getScreenSizeY() - (PaGlobal_CharacterSelect_All._ui.chk_Ghost:GetPosY() - PaGlobal_CharacterSelect_All._ui.stc_RightBg:GetPosY())
    btnSize = btnSize + (PaGlobal_CharacterSelect_All._ui.list2_Character:GetPosY() - PaGlobal_CharacterSelect_All._ui.stc_RightBg:GetPosY()) + padding
    PaGlobal_CharacterSelect_All._ui.list2_Character:SetSize(PaGlobal_CharacterSelect_All._ui.list2_Character:GetSizeX(), getScreenSizeY() - btnSize)
  end
...
function PaGlobalFunc_CharacterSelect_All_List2Update(content, key)
  ...
  if charIdx < haveCount then
    ...
    if nil ~= charData then
      ...
      if __eTUserCharacterNoDefault ~= charData._duelCharacterNo then
...
function HandleEventOn_CharacterSelect_All_DuelCharTooltip(isShow, charDataIdx)
  ...
  local duelCharacterIndex = ToClient_GetDuelCharacterIndexByCharacterNo(characterData._duelCharacterNo)
  local duelCharacterData = getCharacterDataByIndex(duelCharacterIndex)
  if nil == duelCharacterData then
    return
  end
  local name = PAGetString(Defines.StringSheet_GAME, "LUA_GAME_DUELCHARICON_TOOLTIP_NAME")
  local desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GAME_DUELCHARICON_TOOLTIP_DESC", "Charactername", getCharacterName(duelCharacterData))
...
function HandleEventOn_CharacterSelect_All_GetServantTooltipText(briefServantInfo)
  ...
  if CppEnums.ServantKind.Type_Horse == servantKind or CppEnums.ServantKind.Type_RaceHorse == servantKind then
    ...
    if tier > 0 then
      if tier >= 9 then
        if 31539 == briefServantInfo:getServantCharacterKeyRaw() then
          tier = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_DREAMHORSE_GRADE")
        else
...
function HandleEventLUp_CharacterSelect_All_PrepareEnterToField(charIdx)
  ...
  if true == ToClient_isEventChannel() and false == ToClient_SelfPlayerIsGM() then
    if false == characterData._isEventCharacter then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_CANNOT_ACCESS"))
      return
    end
    if true == ToClient_isDiffServerNo(characterData._lastEnterServerNo) then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_OTHERSERVER_ACCESS"))
      return
    end
  end
