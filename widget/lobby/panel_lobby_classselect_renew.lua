...
function ClassSelect:open()
  ...
  local classType = -1
  local topClassType
  local isSpecialCharacter = FGlobal_getIsSpecialCharacter()
  local count = getPossibleClassCount()
  local newClassType = __eClassType_Sorcerer_Reserved1
  for classIdx = 0, count - 1 do
    classType = getPossibleClassTypeFromIndex(classIdx)
    if false == isSpecialCharacter and classType == newClassType and nil ~= self:getClassButtonData(classType) then
      self._ui.list_ClassSelect:getElementManager():pushKey(toInt64(0, classType))
  ...
  for classIdx = 0, count - 1 do
    classType = getPossibleClassTypeFromIndex(classIdx)
    if nil ~= self:getClassButtonData(classType) and (true == isSpecialCharacter or classType ~= newClassType) then
      self._ui.list_ClassSelect:getElementManager():pushKey(toInt64(0, classType))
      if nil == topClassType then
        topClassType = classType
      end
...
function ClassSelect:getClassButtonData(classType)
  ...
  elseif classType == __eClassType_Guardian then
    ...
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_BATTLEAXE")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SHIELD")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_WP")
  elseif classType == __eClassType_Nova then
    ...
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_STAR")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_KRATOOM")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_SP")
  elseif classType == __eClassType_ReservedBlackSpirit then
    ...
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_KAIVE")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_TALISMAN")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_EP")
  elseif classType == __eClassType_Sorcerer_Reserved1 then
    ...
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SERENAKA")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_MAREKA")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_EP")
  else
    buttonData = nil
  end
...
function PaGlobalFunc_ClassSelect_CharacterCreate(characterName)
  ...
  local function createCharacterFunc()
    ...
    chracterCreate(self._currentSelectType, _edit_CharacterName, _isSpecialCharacter, isSeasonCharacter)
