...
function PaGlobal_GlobalKeyBinder.Process_GameMode()
  ...
  if true == _ContentsGroup_RenewUI_Housing then
    ...
    local canUseInstallationMode = false == isVisitHousePS and false == ToClient_isPS4() or nil ~= PaGlobalFunc_HousingName_isMyHouse and true == PaGlobalFunc_HousingName_isMyHouse()
...
function PaGlobal_GlobalKeyBinder.Process_UIMode_NpcDialog(deltaTime)
  ...
  if not getEscHandle() and GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
    ...
    if nil ~= Panel_Window_Wanted and nil ~= Panel_Window_Wanted_Set and false == Panel_Window_Wanted_Set:GetShow() and true == Panel_Window_Wanted:GetShow() then
  ...
  if false == _ContentsGroup_RenewUI_DeadMessage then
    if true == GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
      ...
      if true == _ContentsGroup_WikiNoToURL then
        if nil ~= Panel_Window_ExpirienceWiki_All and true == Panel_Window_ExpirienceWiki_All:GetShow() and nil ~= InputMLUp_ExpirienceWiki_All_Close then
...
function PaGlobal_GlobalKeyBinder.Process_UIMode_InGameCashShop(delataTime)
  ...
  if true == _ContentsGroup_PearlShopMileage and true == _ContentsGroup_NewUI_MoonBlessing_All and nil ~= PaGlobal_CashMileage_IsOpenCheck and true == PaGlobal_CashMileage_IsOpenCheck() and isPadPressed(__eJoyPadInputType_RightTrigger) and isPadPressed(__eJoyPadInputType_Y) then
...
function PaGlobal_GlobalKeyBinder.Process_UIMode_Gacha_Roulette(deltaTime)
  if true == _ContentsGroup_UsePadSnapping then
    ...
    if nil ~= ItemBoxGoXXX and true == ItemBoxGoXXX() then
      _PA_LOG("\234\180\145\236\154\180", "2222222222")
      HandleEventLUp_RandomBoxSelect_All_OnPressSpace()
    end
    if GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_SPACE) then
      if true == _ContentsGroup_NewUI_Gacha_All then
        _PA_LOG("\234\180\145\236\154\180", "3333")
        HandleEventLUp_RandomBoxSelect_All_OnPressSpace()
      else
        _PA_LOG("\234\180\145\236\154\180", "4444")
        FGlobal_gacha_Roulette_OnPressSpace()
      end
  ...
  if true == Panel_Window_CutSceneSkip:GetShow() then
    ...
    if true == Panel_Window_CutSceneScript:GetShow() then
      ...
      if self._skipState.Press_Any_Key == self._currentSkipState then
        if true == self:isKeyDown_Once() then
          self._ui._stc_skipAnno:SetShow(true)
          self._currentSkipState = self._skipState.Press_R_Key
        end
      else
        if self._skipState.Press_R_Key == self._currentSkipState then
