...
function PaGlobal_Achievement_BookShelf_All:setBookShelf()
  ...
  for index = 1, nowJournalMaxKey * self._BOOK_AREA_CNT do
    ...
    while true do
      ...
      elseif nil ~= ToClient_GetJournalName(callIndex) or callIndex > self._MAX_CALLING_COUNT then
        break
      end
...
function PaGlobal_Achievement_BookShelf_All:setChapterSlot()
  ...
  for index = 1, self._journalChapterCnt do
    ...
    if self._journalChapterCnt > 0 then
      ...
      PaGlobal_Achievement_BookShelf_All:SetBookTexture(self._selectedJournalGroup, chapterSlot, chapterKey)
      ...
      if 9 == self._selectedJournalGroup then
        chapterSlot:addInputEvent("Mouse_On", "HandleEventOnOut_Achievement_BookShelf_All_Show_EventJournal_Title(" .. chapterKey .. ")")
