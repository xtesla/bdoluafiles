...
function FGlobal_IngameCashShop_BuyOrGift_SetPearlStampValue(price, moneyType)
  ...
  if false == self._static_CouponApplyBG:GetShow() and stampCount <= userStampCount and true == isOpenPearlStamp and moneyType ~= UI_CCC.eCashProductCategory_Pearl and moneyType ~= UI_CCC.eCashProductCategory_Mileage then
    ...
    if nil ~= guildGiveawayBG then
      ...
      addSizeY = sizeY + 10
      local posY = self._stc_guildGiveawayItemBG:GetPosY()
      guildGiveawayBG:SetPosY(posY)
      self._txt_guildGiveawayDesc:SetPosY(posY + sizeY - self._txt_guildGiveawayDesc:GetSizeY())
    end
    Panel_IngameCashShop_BuyOrGift:SetSize(Panel_IngameCashShop_BuyOrGift:GetSizeX(), buyConfig._startY + buyConfig._gapX + self._config._sizePanel + self._static_PearlBG:GetSizeY() + 40 + addSizeY)
...
function InGameShopBuy_ConfirmDo()
  ...
  ToClient_requestBuyItem(self._productNoRaw, count, cashProduct:getPrice(), isImmediatelyUse, couponNo, couponKey, pearlCount, fromWhereType, true)
  InGameShopBuy_Close()
end
