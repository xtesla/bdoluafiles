...
local __eCategoryType_GuildGiveaway = 13
local __eCategoryType_PearlBox = 4
...
function inGameShop:init()
  ...
  local newClassType = __eClassType_Sorcerer_Reserved1
  local newClassContentsGroupOpen = ToClient_IsContentsGroupOpen("1589")
  for ii = 0, classCount - 1 do
    local classType = getCharacterClassTypeByIndex(ii)
    local className = getCharacterClassName(classType)
    if nil ~= className and "" ~= className and " " ~= className and (newClassType ~= classType or true == newClassContentsGroupOpen) then
  ...
  for ii = 0, maxSlotCount - 1 do
    ...
    slot.eventCartIcon:addInputEvent("Mouse_On", "IngameCashShop_ShowEventIconEffect()")
      ...
      if self._currentPos < pos + slot.static:GetSizeY() + self._goodDescBG:GetSizeY() and pos + slot.static:GetSizeY() < self._currentPos + areaSizeY and 1 < self._goodDescBG:GetSizeY() then
        ...
        if true == _ContentsGroup_SelectBox and false == isGameServiceTypeConsole() then
          if true == self.desc._btn_BigGift:GetShow() then
            self.desc._btn_SideDetail:SetSpanSize(self.desc._btn_BigGift:GetSpanSize().x + self.desc._btn_SideDetail:GetSizeX(), 10)
          else
            self.desc._btn_SideDetail:SetSpanSize(self.desc._btn_BigCart:GetSpanSize().x + self.desc._btn_SideDetail:GetSizeX(), 10)
          end
        end
      end
...
function inGameShop:initTabPos()
  ...
  if nil ~= listPos[__eCategoryType_PearlBox] and nil ~= listPos[__eCategoryType_GuildGiveaway] then
    local tmpKey = -1
    for key, value in pairs(listPos) do
      if __eCategoryType_PearlBox == key then
        tmpKey = __eCategoryType_GuildGiveaway
      elseif tmpKey > 0 then
        self._tabs[tmpKey].static:SetPosY(value)
        if __eCategoryType_GuildGiveaway == key then
          break
        end
        tmpKey = key
      end
...
function setPosOpenSubTab(tabIndex)
  ...
  if nil ~= listPos[__eCategoryType_GuildGiveaway] then
    if nil ~= listPos[__eCategoryType_PearlBox] then
      local tmpKey = -1
      for key, value in pairs(listPos) do
        if __eCategoryType_PearlBox == key then
          tmpKey = __eCategoryType_GuildGiveaway
    ...
    elseif true == self._tabs[__eCategoryType_PearlBox].static:GetShow() then
      local posY = self._tabs[__eCategoryType_PearlBox].static:GetPosY() + tabConfig._gapY
      self._tabs[__eCategoryType_GuildGiveaway].static:SetPosY(posY)
    end
  end
...
function IngameCashShop_PearlBox_Open(slotNo, itemWrapper, count, inventoryType)
  ...
  elseif nil ~= PaGlobalFunc_CashMileage_GetShow and PaGlobalFunc_CashMileage_GetShow() then
...
function IngameCashShop_DescUpdate()
  ...
  for _, slotIcon in pairs(inGameShop._static_EquipSlots) do
    slotIcon._slotIcon:SetPosY(optionDesc_PosY + descConfig._gapY * descCount)
  end
...
function IngameCashShop_SelectedItem(index, bValue)
  ...
  if true == _ContentsGroup_SelectBox and false == isGameServiceTypeConsole() then
    ...
    if nil ~= cashProduct then
      local isHaveSelectBoxItem = cashProduct:isHaveSelectBoxItem()
      if true == isHaveSelectBoxItem and nil ~= Panel_Window_SelectBox then
        InGameShop_ProcessSideDetailFunc(slot.productNoRaw)
        if true == Panel_Window_SelectBox:GetShow() and true == self._isClick then
          PaGlobal_SelectBox:prepareClose()
        end
      end
    end
  ...
  if true == self._hasMyClassItem then
...
function InGameCashshopDescUpdate(deltaTime)
  ...
  for key, slot in pairs(inGameShop._static_EquipSlots) do
    ...
    if nil ~= cashProduct and false == cashProduct:isChooseCash() and true == cashProduct:isShowSlotIcon(slot._slotNo) then
      slotIcon:SetShow(slotIcon:GetPosY() + slotIcon:GetSizeY() > 0 and slotIcon:GetPosY() + slotIcon:GetSizeY() < self._goodDescBG:GetSizeY())
    else
...
function PaGlobal_InGameCashShop_CheckClose()
  ...
  if true == _ContentsGroup_NewUI_MoonBlessing_All and nil ~= PaGlobalFunc_IngameCashShopMoonBlessing_All_CheckMileageRewarded then
...
function PaGlobalFunc_InGameCashShop_PromotionBannerUrlOldSet()
  ...
  elseif isGameTypeKR2() then
    ...
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_KR2")
...
function PaGlobalFunc_IngameCashShop_isActualUsableClass(cashProduct)
  ...
  local isUseableClass = itemSS._usableClassType:isOn(selfPlayer:getClassType())
