...
function PaGlobal_MusicBoard_All_ListTooltipShow(key, index)
  ...
  local control = UI.getChildControl(content, "Radiobutton_Instrument_" .. index % PaGlobal_MusicBoard_All._controlCount + 1)
  if nil == control then
    return
  end
  TooltipSimple_Show(control, PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. index + PaGlobal_MusicBoard_All._current_InstrumentType * PaGlobal_MusicBoard_All._controlCount))
end
