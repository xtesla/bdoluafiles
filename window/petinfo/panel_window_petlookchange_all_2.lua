...
function PaGlobal_PetLookChange_Request_All(petIndex, actionIndex)
  ...
  local function changePetLook()
    ToClient_requestPetChangeLook(petData._petNo, actionIndex, PaGlobal_PetLookChange_All._whereType, PaGlobal_PetLookChange_All._slotNo)
  end
  local petName = petData:getName()
  local messageContent = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_PETLOOKCHANGE_CONFIRM_MESSAGE", "PetName", petName)
  local messageboxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS"),
    content = messageContent,
    functionYes = changePetLook,
