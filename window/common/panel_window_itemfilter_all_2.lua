...
function HandleEventMOn_ItemFilter_IconOver(index)
  ...
  if index <= PaGlobal_ItemFilter_All._familyInventoryItemCount then
...
function HandleEventMLUp_ItemFilter_ItemUse()
  ...
  local function requestServantSupplyItem(s64_count, slotNo)
    ToClient_requestUseServantSupplyItem(PaGlobal_ItemFilter_All._servantNo, whereType, slotNo, s64_count)
  end
  ...
  if s64_count > Defines.s64_const.s64_1 then
    ...
    ToClient_requestUseServantSupplyItem(PaGlobal_ItemFilter_All._servantNo, whereType, slotNo, 1)
  end
end
