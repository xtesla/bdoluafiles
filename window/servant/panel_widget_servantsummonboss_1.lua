...
function PaGlobal_ServantSummonBoss:commandTextSet()
  ...
  if actionCnt > 0 then
    ...
    self._ui._stc_actionTitle:SetSpanSize(self._ui._stc_actionTitle:GetSpanSize().x, lastSpanY + 20)
    lastSpanY = lastSpanY + 20
    for ii = 1, actionCnt do
      ...
      self._ui._actionList[ii]:SetSpanSize(self._ui._actionList[ii]:GetSpanSize().x, lastSpanY + 20)
      lastSpanY = self._ui._actionList[ii]:GetSpanSize().y
  ...
  if true == self._ui._stc_actionTitle:GetShow() then
    titleSizeY = titleSizeY + self._ui._stc_actionTitle:GetTextSizeY()
  end
  ...
  self._ui._stc_commandBG:SetSize(maxSizeX + 30, titleSizeY + (moveCnt + attackCnt + actionCnt) * 20)
end
