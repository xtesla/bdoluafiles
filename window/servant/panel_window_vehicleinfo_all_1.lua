function PaGlobal_VehicleInfo_All:initialize()
  ...
  self._ui._stc_OnlySpeedBG = UI.getChildControl(stc_basicInfo_right, "Static_OnlySpeedBG")
  self._ui._stc_OnlySpeed = UI.getChildControl(self._ui._stc_OnlySpeedBG, "Static_OnlySpeed")
...
function PaGlobal_VehicleInfo_All:validate()
  ...
  self._ui._stc_OnlySpeedBG:isValidate()
...
function PaGlobal_VehicleInfo_All:updateBasicInfo_rightBg_normal()
  ...
  self._ui._stc_OnlySpeedBG:SetShow(false)
  ...
  if __eVehicleType_Horse == vehicleType or CppEnums.VehicleType.Type_RaceHorse == vehicleType then
    ...
    if __eVehicleType_Horse == vehicleType then
      ...
      if speedAddItemUseCount > 0 then
        ...
        self._ui._stc_OnlySpeedBG:SetShow(true)
      end
    end
  else
...
function PaGlobal_VehicleInfo_All:updateSkill(list2Control, noSkillControl, isNoSkillHide)
  ...
  local skillCountString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANTINFO_SKILLCOUNT", "servantSkillListCnt", listCount - skillDiceCount)
...
function PaGlobal_VehicleInfo_All:updateSkillList(control, key)
  ...
  if nil == skillWrapper and false == _ContentsGroup_HorseSkillLearnNewWay then
    return
  end
  ...
  if nil == skillWrapper and true == _ContentsGroup_HorseSkillLearnNewWay then
    ...
    if true == isStallionSkill then
      local gabX = 5
      skillNameSpanSizeX = skillNameSpanSizeX + stc_skillStallionIcon:GetSizeX() - gabX
      if false == self._isConsole then
