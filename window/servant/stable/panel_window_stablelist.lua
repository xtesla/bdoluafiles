  ...
  if eType == self._const.eTypeSealed then
    ...
    elseif regionName == servantRegionName then
      ...
      if isLinkedHorse and stallionTraining ~= getState then
        ...
        if CppEnums.VehicleType.Type_Horse == servantInfo:getVehicleType() and true == servantInfo:isStallion() and stable_isPossibleStallionSkillExpTraining() and isContentsStallionEnable and isContentsNineTierEnable and isContentsNineTierTraining and false == hasRentOwnerFlag and servantInfo:getLevel() >= 30 and 8 == servantInfo:getTier() and nowMating ~= getState and regMarket ~= getState and regMating ~= getState and training ~= getState then
