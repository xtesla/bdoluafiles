...
function PaGlobal_ChannelSelect_All:createChannelUI()
  ...
  for idx = 0, totalChannelCount - 1 do
    do
      ...
      cloned_ChannelSlot._stc_EventChannel:SetShow(false)
      cloned_ChannelSlot._stc_SeasonChannel:SetShow(false)
      cloned_ChannelSlot._stc_SeasonNewbieChannel:SetShow(false)
      cloned_ChannelSlot._stc_HadumChannel:SetShow(false)
      cloned_ChannelSlot._stc_GuildServer:SetShow(false)
...
function PaGlobal_ChannelSelect_All:createServerDesc()
  ...
  local warDesc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_DESC")
  if true == _ContentsGroup_ConquestSiege then
    warDesc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_DESC_KAMA")
  end
  local descData = {
    [1] = {
...
function PaGlobal_ChannelSelect_All:update()
  ...
  if false == _ContentsGroup_RenewUI then
    ...
    for ch_Index = 0, channelCount - 1 do
      ...
      if nil ~= serverData then
        ...
        if true == _ContentsGroup_SeasonContents and true == serverData._isSeasonChannel or true == serverData._isSpeedChannel then
          isInserted = true
        end
  ...
  for ch_Index = 1, #self._serverIndexTable do
    ...
    if nil == _serverData then
      ...
      if true == _serverData._isEventChannel then
        channelSlot._stc_EventChannel:SetShow(true)
      else
      ...
      if nil ~= guildWrapper then
        ...
        if _serverData._serverGroupNo == guildMainServerGroupNo and false == isVolunteer then
      ...
      elseif _serverData._isSpeedChannel then
        if 0 ~= tempWrapper:getMyAdmissionToSpeedServer() and false == _ContentsGroup_isConsolePadControl then
      ...
      if true == isSiegeBeing and false == isInSiegeBattle then
        isAdmission = true
      elseif _serverData._isEventChannel then
        isAdmission = true
      elseif curChannelData._serverGroupNo == guildMainServerGroupNo and _serverData._serverGroupNo == guildMainServerGroupNo and false == isVolunteer then
        isAdmission = true
