...
function PaGlobal_DailyStamp_All:setData(tabIndex)
  ...
  if true == _ContentsGroup_TransferEventBuff then
    local userConnectionType = self._dailyStampKeys[self._tabIndex][1]:getUserConnectionType()
    local isTransferAccount = ToClient_GetIsTransferAccount()
    if 3 == userConnectionType and false == isTransferAccount then
      local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_TRANSFER_NA_DAILYSTAMP_DESC")
      local messageBoxData = {
        title = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY"),
        content = messageBoxMemo,
        functionYes = PaGlobalFunc_DailyStamp_All_ShowTransferPage,
