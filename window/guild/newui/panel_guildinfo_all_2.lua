...
function PaGlobalFunc_GuildInfo_All_ResetUrl()
  ...
  if true == PaGlobal_GuildInfo_All._isKR2 then
    return
  end
...
function HandleEventLUp_GuildInfo_All_DismissGuild()
  ...
  local isRightDismiss = PaGlobalFunc_GuildInfo_All_RightCheck(__eGuildRightType_GuildDestroy)
...
function HandleEventOnOut_GuildInfo_All_ShowGuildServerTooltip(isShow)
  if false == isShow then
    TooltipSimple_Hide()
    return
  end
  local control = PaGlobal_GuildInfo_All._ui.stc_GuildServer
  local name = PaGlobal_GuildInfo_All._ui.txt_GuildServerName:GetText()
