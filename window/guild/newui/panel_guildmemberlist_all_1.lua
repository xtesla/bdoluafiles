...
function PaGlobal_GuildMemberList_All:updateBottomInfo()
  ...
  local isRightLifeFund = PaGlboalFunc_GuildMemberList_All_GetRightCheck(__eGuildRightType_WelfareMoney)
  if isRightLifeFund and self._GuildGrade.GUILD == guildGrade then
    self._ui.btn_guildLifeFund:SetShow(false == self._isConsole)
  else
    self._ui.btn_guildLifeFund:SetShow(false)
  end
  local isRightFundManage = PaGlboalFunc_GuildMemberList_All_GetRightCheck(__eGuildRightType_PayIncentive)
  self._ui.btn_guildFundManage:SetShow(false)
  if true == isRightFundManage then
