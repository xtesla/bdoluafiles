...
function PaGlobal_GuildSkill_All:openTab()
  ...
  self:initControl()
  if nil ~= PaGlobalFunc_GuildAlliance_All_RegisterUpdateSkillCoolTime then
    self._effectTime = self._effectMaxTime
    PaGlobalFunc_GuildAlliance_All_RegisterUpdateSkillCoolTime()
  end
end
    ...
    if nil ~= skillLevelInfo then
      ...
      if not skillLevelInfo._learnable and skillLevelInfo._usable and skillTypeStaticWrapper:get():isActiveSkill() then
        ...
        if self._isConsole then
          ...
          slot.icon:addInputEvent("Mouse_PressMove", "HandleEventLUp_GuildSkill_StartDrag(" .. skillNo .. ")")
        end
      else
...
function PaGlobal_GuildSkill_All:initControl()
  ...
  self._ui.contents_Tooltip:SetSize(self._ui.contents_Tooltip:GetSizeX(), self._ui.txt_FrameDesc:GetPosY() + self._ui.txt_FrameDesc:GetTextSizeY() + 10)
        ...
        if skillTypeStaticWrapper:isValidLocalizing() then
          ...
          if nil ~= slot.icon then
            if self._isConsole then
              ...
              slot.icon:addInputEvent("Mouse_LUp", "InputEventLUp_GuildSkill_All_LearnSkill(" .. skillNo .. ")")
              if true == skillTypeStatic._isSettableQuickSlot then
...
function PaGlobal_GuildSkill_All:updateToolTip(skillNo)
  ...
  self._ui.txt_LevelName:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_GuildSkill_All:updateGuildSkillCoolTime(skillNo)
  if nil == self.skillNoSlot or nil == self.skillNoSlot[skillNo] then
    return
  end
  if true == self.skillNoSlot[skillNo].cooltime:GetShow() then
    self.skillNoSlot[skillNo].cooltime:SetShow(false)
    self.skillNoSlot[skillNo].cooltimeText:SetShow(false)
  end
  local skillTypeStaticWrapper = getSkillTypeStaticStatus(skillNo)
  ...
  if true == skillLevelInfo._learnable or false == skillLevelInfo._usable or false == skillTypeStaticWrapper:get():isActiveSkill() then
    return
  end
  ...
  local skillKey = skillStaticWrapper:getKey()
  ...
  local intRemainTime = math.ceil(realRemainTime)
  if remainTime > 0 then
    self.skillNoSlot[skillNo].cooltime:UpdateCoolTime(remainTime)
    self.skillNoSlot[skillNo].cooltime:SetShow(true)
    local coolTimeText = Time_Formatting_ShowTop(intRemainTime)
    if nil ~= coolTimeText then
      self.skillNoSlot[skillNo].cooltimeText:SetText(coolTimeText)
    end
    if skillReuseTime >= intRemainTime then
      self.skillNoSlot[skillNo].cooltimeText:SetShow(true)
    else
      self.skillNoSlot[skillNo].cooltimeText:SetShow(false)
    end
  end
end
function PaGlobal_GuildSkill_All:updateTargetScroll(skillNo)
  if false == self._isConsole then
    return
  end
  if nil == self.slots or nil == self.slots[skillNo] then
    return
  end
  local targetControl = self.slots[skillNo].icon
  local content = self._ui.frame_GuildSkill:GetFrameContent()
  local vScroll = self._ui.frame_GuildSkill:GetVScroll()
  if nil == targetControl or nil == content or nil == vScroll then
    return
  end
  local contentSizeY = content:GetSizeY()
  if 0 == contentSizeY then
    return
  end
  local maxBottomPosY = self._ui.frame_GuildSkill:GetSizeY() - targetControl:GetSizeY()
  local targetControlPosY = targetControl:GetPosY()
  local diffPos = content:GetPosY() + targetControlPosY
  if diffPos < 0 or maxBottomPosY < diffPos then
    local newControlPos = targetControlPosY / contentSizeY
    vScroll:SetControlPos(newControlPos)
    self._ui.frame_GuildSkill:UpdateContentPos()
    local x = targetControl:GetParentPosX() + targetControl:GetSizeX() / 2
    local y = targetControl:GetParentPosY() + targetControl:GetSizeY() / 2
    ToClient_setMousePosition(x, y)
    if nil ~= PaGlobalFunc_PadSnapTargetEffect_ChangeTarget then
      PaGlobalFunc_PadSnapTargetEffect_ChangeTarget(targetControl)
    end
  end
end
