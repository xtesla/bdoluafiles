...
function deadMessage:setSize()
  ...
  local baseSizeX = self._ui._button_Template:GetSizeX()
  local baseSizeY = self._ui._button_Template:GetSizeY()
  local startPosY = scrSizeY * 0.65
  local startPosX = scrSizeX * 0.5 - self._ui._button_Respawn[0]:GetSizeX() * 0.5
  local gabX = baseSizeX * 1.5
  local gabY = baseSizeY * 1.2
  ...
  if buttonCount < 5 then
    ...
    for index = 0, buttonCount do
      ...
      buttonControl[index]:SetPosY(startPosY + index * gabY + addPosY)
      local diffSizeY = buttonControl[index]:GetSizeY() - baseSizeY
      if diffSizeY >= 0 then
        addPosY = addPosY + diffSizeY
      end
    end
  else
    local leftAddPosY = 0
    for index = 0, math.floor(buttonCount / 2) - 1 do
      ...
      buttonControl[index]:SetPosY(startPosY + index * gabY + leftAddPosY)
      local diffSizeY = buttonControl[index]:GetSizeY() - baseSizeY
      if diffSizeY >= 0 then
        leftAddPosY = leftAddPosY + diffSizeY
      end
    end
    local rightAddPosY = 0
    for index = math.floor(buttonCount / 2), buttonCount - 1 do
      buttonControl[index]:SetPosX(startPosX + gabX)
      buttonControl[index]:SetPosY(startPosY + (index - math.floor(buttonCount / 2)) * gabY + rightAddPosY)
      local diffSizeY = buttonControl[index]:GetSizeY() - baseSizeY
      if diffSizeY >= 0 then
        rightAddPosY = rightAddPosY + diffSizeY
      end
    end
  end
...
function deadMessage:setRespawnButton()
  ...
  for buttonIndex = 1, #buttonArray do
    ...
    buttonControl[buttonUIIndex]:SetTextMode(__eTextMode_AutoWrap)
    buttonControl[buttonUIIndex]:SetText(buttonString)
    local diffSizeY = buttonControl[buttonUIIndex]:GetTextSizeY() - buttonControl[buttonUIIndex]:GetSizeY()
    if diffSizeY >= 0 then
      buttonControl[buttonUIIndex]:SetSize(buttonControl[buttonUIIndex]:GetSizeX(), buttonControl[buttonUIIndex]:GetSizeY() + addSizeGab + diffSizeY)
    end
    buttonUIIndex = buttonUIIndex + 1
  end
end
...
function deadMessage:buttonPushedEvent_Immediate(useItem, useFairy)
  ...
  if true == _ContentsGroup_FamilyInventory then
    revivalItemCount = revivalItemCount + ToClient_InventorySizeByProductCategory(CppEnums.ItemWhereType.eFamilyInventory, CppEnums.ItemProductCategory.eItemProductCategory_Revival)
  end
  local selfProxy = getSelfPlayer()
  local regionInfo = getRegionInfoByPosition(selfProxy:get():getPosition())
  local isArena = regionInfo:get():isArenaArea()
...
function deadMessage:buttonPushedEvent_Exploration(useItem, useFairy)
  ...
  local function revivalExplorationConfirm()
    ...
    local isUseFairy = false
    if nil ~= useFairy then
      isUseFairy = useFairy
    end
    deadMessage_Revival(self._config._eRespawnType.respawnType_Exploration, 255, 0, getSelfPlayer():getRegionKey(), false, toInt64(0, 0), isUseFairy)
    SetUIMode(Defines.UIMode.eUIMode_Default)
  end
...
function deadMessage:buttonPushedEvent_NearTown(useItem, useFairy)
  ...
  local function revivalVillageConfirm()
    ...
    deadMessage_Revival(self._config._eRespawnType.respawnType_NearTown, 0, 0, getSelfPlayer():getRegionKey(), false, toInt64(0, 0), isUseFairy)
    SetUIMode(Defines.UIMode.eUIMode_Default)
  end
  local messageboxData = {
    title = PAGetString(Defines.StringSheet_GAME, "DEADMESSAGE_TEXT_RESPAWN_MB_TITLE"),
