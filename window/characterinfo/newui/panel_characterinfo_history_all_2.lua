function PaGlobalFunc_CharInfoHistory_All_Update()
  ...
  if true == PaGlobal_CharInfoHistory_All._isConsole then
    local keyGuides = {
      PaGlobal_CharInfoHistory_All._ui.stc_keyGuideB
    }
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuides, PaGlobal_CharInfoHistory_All._ui.stc_keyGuideBG, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
  end
...
function HandleEventLUp_CharInfoHistory_All_ChangeCurrentValue(moveDir)
  if false == PaGlobalFunc_ChracterInfo_All_GetBgShow(PaGlobal_CharInfo_All._TABINDEX._history) then
    return
  end
  PaGlobal_CharInfoHistory_All:changeCurrentValue(moveDir)
end
