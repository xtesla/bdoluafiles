function PaGlobal_WareHouse_All:initialize()
  ...
  self._ui._pc._chk_horseBag = UI.getChildControl(stc_rightBg, "CheckButton_Horse_Bag_PCUI")
  ...
  self._ui._console._txt_keyGuideSilverOrGuildLog = UI.getChildControl(stc_bottomBg, "StaticText_Y")
  ...
  self._ui._console._txt_keyGuideMarketInvenOpen = UI.getChildControl(stc_bottomBg, "StaticText_LT_Y")
  ...
  self._ui._btn_cargoCell = UI.getChildControl(stc_rightBg, "Button_CargoSell")
  self._ui._txt_ItemShare_Desc = UI.getChildControl(stc_rightBg, "StaticText_ItemShare_Desc")
  ...
  self._keyGuide = {
    self._ui._console._txt_keyGuide_RTA,
    self._ui._console._txt_keyGuide_RTX,
    ...
    self._ui._console._txt_keyGuideSilverOrGuildLog,
...
function PaGlobal_WareHouse_All:registEventHandler()
  ...
  if true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI then
    Panel_Window_Warehouse:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "PaGlobalFunc_MarketWallet_Open(" .. CppEnums.ItemWhereType.eWarehouse .. ")")
  end
...
function PaGlobal_WareHouse_All:update()
  ...
  if true == self._isSorted then
    if true == isGuildWare then
      local wholeTable = Array.new()
      local shareItemTable = Array.new()
      local guildItemTable = Array.new()
      local emptyTable = Array.new()
      for ii = useStartSlot, useMaxCount - 1 do
        local invenItem = PaGlobal_Warehouse_All_GetItem(ii)
        if nil ~= invenItem then
          local itemWrapper = invenItem:getStaticStatus()
          if nil ~= itemWrapper then
            local isGuildRentalItem = itemWrapper:get():getEnchant()._isGuildRentalAble
            if true == isGuildRentalItem then
              shareItemTable:push_back(ii)
            else
              guildItemTable:push_back(ii)
            end
          end
        else
          emptyTable:push_back(ii)
        end
      end
      shareItemTable:quicksort(PaGlobal_Warehouse_All_ItemComparer)
      guildItemTable:quicksort(PaGlobal_Warehouse_All_ItemComparer)
      if CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildItemShare == PaGlobal_WareHouse_All._fromType then
        for idx = 1, #shareItemTable do
          wholeTable:push_back(shareItemTable[idx])
        end
        for idx = 1, #guildItemTable do
          wholeTable:push_back(guildItemTable[idx])
        end
      elseif CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildHouse == PaGlobal_WareHouse_All._fromType then
        for idx = 1, #guildItemTable do
          wholeTable:push_back(guildItemTable[idx])
        end
        for idx = 1, #shareItemTable do
      ...
      for idx = 1, #emptyTable do
        wholeTable:push_back(emptyTable[idx])
      end
      for ii = 1, useMaxCount - 1 do
        slotNoList[ii] = wholeTable[ii]
      end
    else
  ...
  for ii = 0, self._config.slotCount - 1 do
    ...
    elseif ii == useMaxCount - useStartSlot - self._startSlotIndex and ii < maxCount - useStartSlot - self._startSlotIndex then
      if self._slots[ii].icon:GetShow() then
        ...
        if true == self:isNpc() or true == self:isWorldMapNode() then
...
function PaGlobal_WareHouse_All:updateAssetManagement(slotNoList, useMaxCount, money_s64)
  if true == self:isGuildHouse() or true == self:isGuildItemShare() then
    return
  end
  local warehouseWrapper = self:getWarehouse()
  if nil == warehouseWrapper then
...
function PaGlobal_WareHouse_All:validate()
  ...
  self._ui._console._txt_keyGuideSilverOrGuildLog:isValidate()
...
function PaGlobal_WareHouse_All:resizePanelChange(isNpc)
  ...
  if not self:isGuildHouse() and not self:isGuildItemShare() then
  ...
  elseif true == self:isGuildItemShare() and true == _ContentsGroup_GuildShareItemWareHouse then
    ...
    self._ui._stc_rightBg:SetSize(self._defaultRightBgSize.x, self._defaultRightBgSize.y - moneybgSizeY + descSizeY)
  ...
  if PaGlobal_WareHouse_All:isNpc() then
    ...
    if true == _ContentsGroup_MoveNonStackItem then
      ...
      if false == itemSSW:isStackable() and s64_count > toInt64(0, 1) then
        ToClient_pushNonStackItemToWareHouse(PaGlobal_WareHouse_All._targetActorKeyRaw, false, whereType, s64_count)
      else
    ...
    if warehouseInMaid then
      ...
      if true == _ContentsGroup_UseMultiMaid and true == self._isUseMultiMaid then
        ...
        if true == _ContentsGroup_MoveNonStackItem and false == itemSSW:isStackable() and s64_count > toInt64(0, 1) then
          ToClient_pushNonStackItemToWareHouse(PaGlobal_WareHouse_All._targetActorKeyRaw, true, whereType, s64_count)
          return
        end
        ...
        warehouse_pushFromInventoryItemByMaid(whereType, slotNo, s64_count, PaGlobal_WareHouse_All._targetActorKeyRaw, moveCountOnce)
      else
        warehouse_pushFromInventoryItemByMaid(whereType, slotNo, s64_count, PaGlobal_WareHouse_All._targetActorKeyRaw, moveCountOnce)
      end
    else
...
function PaGlobal_WareHouse_All:popToSomewhere(s64_count, slotNo)
  ...
  if invenIsShow and GetUIMode() ~= Defines.UIMode.eUIMode_WorldMap then
    if self:isNpc() then
      ...
      if true == _ContentsGroup_MoveNonStackItem and false == itemSSW:isStackable() and s64_count > toInt64(0, 1) then
        ToClient_popNonStackItemFromWarehouse(self._targetActorKeyRaw, false, s64_count)
        return
      end
    ...
    elseif self:isMaid() then
      ...
      if warehouseOutMaid then
        ...
        if true == _ContentsGroup_UseMultiMaid and true == self._isUseMultiMaid then
          ...
          if false == itemSSW:isMoney() then
            if true == _ContentsGroup_MoveNonStackItem and false == itemSSW:isStackable() and s64_count > toInt64(0, 1) then
              ToClient_popNonStackItemFromWarehouse(self._targetActorKeyRaw, true, s64_count)
              return
            end
          ...
          warehouse_popToInventoryByMaid(slotNo, s64_count, self._targetActorKeyRaw, moveCountOnce)
        else
          warehouse_popToInventoryByMaid(slotNo, s64_count, self._targetActorKeyRaw, moveCountOnce)
        end
      else
...
function PaGlobal_WareHouse_All:openPanel(waypointKey, fromType)
  ...
  if CppEnums.WarehoouseFromType.eWarehoouseFromType_Npc == self._fromType and true == _ContentsGroup_CargoSell then
  ...
  local regionInfo = getRegionInfoByPosition(getSelfPlayer():get():getPosition())
  if hasItemMarket and nil ~= dialog_getTalker() and dialog_isTalking() and not self:isGuildHouse() and not self:isGuildItemShare() then
    self._buttonData.marketRegist = CppEnums.WarehoouseFromType.eWarehoouseFromType_Npc == self._fromType
  else
    self._buttonData.marketRegist = false
  end
  if true == _ContentsGroup_RenewUI and true == self:isInstallation() then
    self._buttonData.marketRegist = false
  end
  ...
  if CppEnums.WarehoouseFromType.eWarehoouseFromType_Npc == self._fromType then
    if self._ui._btn_itemMarketRegist:GetShow() then
      if self._ui._btn_manufacture:GetShow() then
        if true == _ContentsGroup_CargoSell then
          local gap = 10
          local buttonSizeX = (stc_moneyBg:GetSizeX() - gap * 2) / 3
          ...
          self._ui._btn_manufacture:SetSpanSize(gap * 2 + buttonSizeX, self._btnMarketRegSpanSizeY + self.addSizeY)
          self._ui._btn_cargoCell:SetSize(buttonSizeX, self._ui._btn_itemMarketRegist:GetSizeY())
          self._ui._btn_cargoCell:SetSpanSize(gap, self._btnMarketRegSpanSizeY + self.addSizeY)
          self._ui._btn_cargoCell:SetShow(true)
          self._ui._btn_itemMarketRegist:SetSpanSize(buttonSizeX * 2 + gap * 3, self._btnMarketRegSpanSizeY + self.addSizeY)
        else
      ...
      elseif true == _ContentsGroup_CargoSell then
        local buttonSizeX = (stc_moneyBg:GetSizeX() - 10) * 0.5
        self._ui._btn_itemMarketRegist:SetSize(buttonSizeX, self._ui._btn_itemMarketRegist:GetSizeY())
        self._ui._btn_cargoCell:SetSize(buttonSizeX, self._ui._btn_itemMarketRegist:GetSizeY())
        self._ui._btn_cargoCell:SetSpanSize(10, self._btnMarketRegSpanSizeY + self.addSizeY)
        self._ui._btn_cargoCell:SetShow(true)
        self._ui._btn_itemMarketRegist:SetSpanSize(buttonSizeX + 20, self._btnMarketRegSpanSizeY + self.addSizeY)
      else
        self._ui._btn_cargoCell:SetShow(false)
    ...
    elseif true == self._ui._btn_manufacture:GetShow() then
      if true == _ContentsGroup_CargoSell then
        local buttonSizeX = (stc_moneyBg:GetSizeX() - 10) * 0.5
        self._ui._btn_cargoCell:SetSize(buttonSizeX, self._ui._btn_itemMarketRegist:GetSizeY())
        self._ui._btn_manufacture:SetSize(buttonSizeX, self._ui._btn_itemMarketRegist:GetSizeY())
        self._ui._btn_manufacture:SetSpanSize(10, self._btnMarketRegSpanSizeY + self.addSizeY)
        self._ui._btn_cargoCell:SetSpanSize(buttonSizeX + 20, self._btnMarketRegSpanSizeY + self.addSizeY)
      else
      end
    elseif true == _ContentsGroup_CargoSell then
      self._ui._btn_cargoCell:SetSize(stc_moneyBg:GetSizeX(), self._ui._btn_itemMarketRegist:GetSizeY())
      self._ui._btn_cargoCell:SetSpanSize(10, self._btnMarketRegSpanSizeY + self.addSizeY)
    end
  else
...
function PaGlobal_WareHouse_All:openWithInventory(filterFunc, isScrollHold)
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    if false == Panel_Window_Inventory_All:GetShow() and nil ~= InventoryWindow_Show then
      InventoryWindow_Show()
    end
  elseif false == Panel_Window_Inventory:GetShow() and nil ~= InventoryWindow_Show then
    InventoryWindow_Show()
  end
...
function PaGlobal_WareHouse_All:updateButtonInfo()
  if true == self._isConsole then
    ...
    self._ui._console._txt_keyGuideSilverOrGuildLog:SetShow(self._buttonData.sendMoney or self._buttonData.guildLog)
    ...
    elseif Defines.UIMode.eUIMode_NpcDialog == GetUIMode() then
      Panel_Window_Warehouse:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "PaGlobalFunc_MarketPlace_OpenFromDialog()")
    else
      Panel_Window_Warehouse:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "PaGlobalFunc_MarketPlace_Open()")
    end
...
function PaGlobal_WareHouse_All:setInventoryItemMoveEvent(isSet)
  ...
  if true == isSet then
    Panel_Window_Warehouse:registerPadEvent(__eConsoleUIPadEvent_RTPress_X, "PaGlobalFunc_Inventory_RequestMoveItemsInventoryToWarehouse()")
    Panel_Window_Warehouse:registerPadEvent(__eConsoleUIPadEvent_RTPress_A, "PaGlobal_ItemMoveSet:prepareOpen()")
    ...
    self._ui._console._txt_keyGuide_RTA:SetShow(isInvenShow)
    self._ui._console._txt_keyGuide_RTX:SetShow(isInvenShow)
  else
