...
function Panel_Window_StroyInstanceDungeon_info:initLoadMovie()
  ...
  if nil == self._ui.cutSceneMovie then
    if true == _ContentsGroup_UseInGameVideoContent then
      self._ui.cutSceneMovie = UI.createControl(__ePAUIControl_VideoContent, Panel_MovieStroyIndun, "VideoControl_Movie")
    else
  ...
  if nil ~= self._ui.cutSceneMovie then
    ...
    if true == _ContentsGroup_UseInGameVideoContent then
      local moviePath = "UI_Movie/Bink/Kzaka/CS_Calpheon_01_SealingKzaka.bk2"
      local subTitlePath = "UI_Movie/Bink/Kzaka/CS_Calpheon_01_SealingKzaka.srt"
      self._ui.cutSceneMovie:createVideoView(moviePath, getResolutionSizeX(), getResolutionSizeY(), false, true, subTitlePath)
      self._value.masterVolum = getVolumeParam(0)
      setVolumeParamMaster(0)
      self._value.isPlayMovie = true
      self._value.setvolume = true
      registerEvent("ToClient_EndGuideMovie", "FromClient_StroyInstanceDungeon_InGameMovieCutScene")
    else
...
function Panel_Window_StroyInstanceDungeon_info:stop()
  ...
  self._value.isPlayMovie = false
  self._value.setvolume = false
  if nil ~= self._ui.cutSceneMovie and false == _ContentsGroup_UseInGameVideoContent then
    self._ui.cutSceneMovie:TriggerEvent("StopMovie", "")
    self._ui.cutSceneMovie:ResetUrl()
  elseif true == _ContentsGroup_UseInGameVideoContent then
    self._ui.cutSceneMovie:resetVideoView()
    unregisterEvent("ToClient_EndGuideMovie", "FromClient_StroyInstanceDungeon_InGameMovieCutScene")
  end
end
