...
function PaGlobal_SpiritEnchant_All:setProtectorItems()
  local itemWrapper = getInventoryItemByType(self._grantItemWhereType, self._grantItemSlotNo)
  local enchantItemClassify = itemWrapper:getStaticStatus():getItemClassify()
  local enchantLevel = itemWrapper:get():getKey():getEnchantLevel()
  local enchantType = self._enchantInfo:ToClient_getEnchantType()
  ...
  if false == isStackLessBlackStone and false == self._isSetNewPerfectItemMaterial then
    local hasProtectorItem = self._enchantInfo:ToClient_hasProtectorItemInInventory()
    if false == hasProtectorItem then
      isDefenceFail = false
...
function PaGlobal_SpiritEnchant_All:setCronEnchant()
  ...
  local dec_MaxEndura = self._enchantInfo:ToClient_getDecMaxEndura()
  local isStackLessBlackStone = itemWrapper:getStaticStatus():isNeedStackLessBlackStonForEnchant()
  if (enchantLevel > 16 or 4 == enchantItemClassify) and false == isStackLessBlackStone and toInt64(0, 0) < self._enchantInfo:ToClient_getNeedCountForProtect_s64() and false == self._isSetNewPerfectItemMaterial then
...
function PaGlobal_SpiritEnchant_All:startEnchant()
  ...
  self._enchantInfo:ToClient_doEnchant(self._ui_enchant.chk_enchantForce:IsCheck(), isDifficulty(), isProtector())
...
function PaGlobal_SpiritEnchant_All:retargetUI()
  local itemWrapper = getInventoryItemByType(self._grantItemWhereType, self._grantItemSlotNo)
  if nil == itemWrapper then
    _PA_ASSERT(false, "\236\151\134\235\139\164 \236\149\132\236\157\180\237\133\156\236\157\180")
    return
  end
