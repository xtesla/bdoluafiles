...
function InventoryInfo:initFamilyInventory()
  self._ui.stc_FamilyinvenSlotArea = UI.getChildControl(self._ui.stc_familyInvenGroup, "Static_FamilyInvenBG")
  self._ui.txt_keyGuideFamilyInvenSort = UI.getChildControl(self._ui.stc_familyInvenGroup, "StaticText_Sort_ConsoleUI")
...
function InventoryInfo:registEventHandler()
  ...
  _panel:registerPadEvent(__eConsoleUIPadEvent_RT, "PaGlobal_FamilyInvnetoryShowToggle()")
...
function InventoryInfo:getRealFamilyInvenSlotNo(slotIndex)
  if false == ToClient_IsSortedFamilyInventory() then
  ...
  return selfPlayerWrapper:get():getRealInventorySlotNo(CppEnums.ItemWhereType.eFamilyInventory, slotIndex + self._familyInvenStartSlotIndex)
end
function InventoryInfo:updateFamilyInventory()
  if false == self._initialize then
    return
  end
  if false == self._ui.stc_familyInvenGroup then
    return
  end
  local selfPlayerWrapper = getSelfPlayer()
  if nil == selfPlayerWrapper then
...
function InventoryInfo:updateInventory(ignorePanelVisibility)
  ...
  for ii = 1, self._invenSlotCount do
    ...
    if true == self._isAutoSort then
      slotNo = selfPlayer:get():getRealInventorySlotNo(currentWhereType, ii + self._invenStartSlotIndex - 1)
    end
    slot:clearItem()
    slot.slotNo = slotNo
    ...
    if nil ~= itemWrapper and nil ~= slotNo then
      ...
      if true == itemSSW:isEquipable() then
        ...
        if not isUsableServant and not Panel_Window_Exchange:GetShow() and nil ~= defencePoint and nil ~= offencePoint and currentEndurance > 0 and true == matchEquip and defencePoint + offencePoint > equipDefencePoint + equipOffencePoint then
...
function PaGlobalFunc_Window_InventoryInfo_PerFrameUpdate(deltaTime)
  ...
  if true == self._isShowFamilyInven then
    for ii = 0, self._familyinvenShowSlotCount - 1 do
      ...
      if remainTime > 0 then
        itemReuseTime = getItemReuseCycle(CppEnums.ItemWhereType.eFamilyInventory, slotNo) / 1000
...
function Input_InventoryInfo_SetLowerTabLeft()
  ...
  if true == ToClient_IsSortedInventory() then
    ...
    selfPlayer:sortInventorySlotNo(Inventory_GetCurrentInventoryType())
  end
  self:updateInventory()
end
...
function InputRSClick_InventoryInfo_OpenCraftNote(index, isFamilyInven)
  ...
  if false == isFamilyInven then
    inventoryType = _lowerTabData[self._currentLowerTab].whereType
...
function Input_InventoryInfo_AutoSort()
  ...
  if false == self._isShowFamilyInven then
    ...
    whereType = WHERE_TYPE.FAMILY_INVENTORY
    ToClient_SetSortedFamilyInventory(not isSorted)
    if false == isSorted then
      self._ui.txt_keyGuideFamilyInvenSort:SetText(_keyGuideData.InventoryInfo_InvenSlotBG_[KEY_GUIDE_TYPE.LS_CLICK]._unsort)
      local selfPlayer = getSelfPlayer():get()
      selfPlayer:sortInventorySlotNo(whereType)
    else
      self._ui.txt_keyGuideFamilyInvenSort:SetText(_keyGuideData.InventoryInfo_InvenSlotBG_[KEY_GUIDE_TYPE.LS_CLICK]._sort)
    end
...
function PaGlobal_FamilyInvnetoryShowToggle()
  ...
  if false == self._isShowFamilyInven then
    ...
    local isSetSorting = ToClient_IsSortedFamilyInventory()
    if true == isSetSorting then
      selfPlayerGet:sortInventorySlotNo(WHERE_TYPE.FAMILY_INVENTORY)
    end
    self:updateFamilyInventory()
...
function Input_InventoryInfo_FamilyInvenFloatingTooltip(index, isShow)
  ...
  if false == _ContentsGroup_RenewUI_Tooltip then
    ...
    local itemWrapper = getInventoryItemByType(WHERE_TYPE.FAMILY_INVENTORY, InventoryInfo._familyInventoryTooltipSlotNo)
    if nil ~= itemWrapper then
...
function InputMRUp_InventoryInfo_FamilyInvenSlot_UseItem(index)
  ...
  local slotNo = self:getRealFamilyInvenSlotNo(index)
  if DragManager:isDragging() then
    if DragManager.dragWhereTypeInfo == CppEnums.ItemWhereType.eFamilyInventory and false == ToClient_IsSortedFamilyInventory() then
...
function InputMRUp_InventoryInfo_FamilyInvenSlot_MoveItemToFamilyInventory(index)
  if false == InventoryInfo._isShowFamilyInven then
    return
  end
  local slotNo = InventoryInfo._ui.slot_inven[index].slotNo
  local itemCount = 0
...
function InputMRUp_InventoryInfo_FamilyInvenSlot_MoveItemFromFamilyInventory(index)
  local slotNo = InventoryInfo:getRealFamilyInvenSlotNo(index)
  local itemWhereType = CppEnums.ItemWhereType.eFamilyInventory
  local itemWrapper = getInventoryItemByType(itemWhereType, slotNo)
  if nil == itemWrapper then
    return
...
function FromClient_InventoryInfo_updateInvenSlot()
  InventoryInfo:updateInventory()
  InventoryInfo:updateWeight()
  InventoryInfo:updateFamilyInventory()
end
...
function Input_InventoryInfo_InvenSlotKeyGuide()
  ...
  if false == self._isShowFamilyInven then
    ...
    local isSorted = ToClient_IsSortedFamilyInventory()
    if false == isSorted then
      self._ui.txt_keyGuideFamilyInvenSort:SetText(_keyGuideData.InventoryInfo_InvenSlotBG_[KEY_GUIDE_TYPE.LS_CLICK]._sort)
    else
      self._ui.txt_keyGuideFamilyInvenSort:SetText(_keyGuideData.InventoryInfo_InvenSlotBG_[KEY_GUIDE_TYPE.LS_CLICK]._unsort)
...
function PaGlobalFunc_Equipment_IsReuseTime(deltaTime)
  if 3 <= InventoryInfo._reuseAlchemyStoneCheckTime then
...
function PaGlobalFunc_Inventory_RequestMoveItemsInventoryToWarehouse()
...
function PaGlobalFunc_Inventory_All_familyInventoryButtonActive(isActive)
  ...
  if true == isActive then
    InventoryInfo._familyInventoryIgnoreCount = InventoryInfo._familyInventoryIgnoreCount - 1
  else
    InventoryInfo._familyInventoryIgnoreCount = InventoryInfo._familyInventoryIgnoreCount + 1
  end
  if InventoryInfo._familyInventoryIgnoreCount > 0 then
    PaGlobal_FamilyInvnetory_Close(false)
  else
    InventoryInfo._familyInventoryIgnoreCount = 0
  end
end
function PaGlobalFunc_Inventory_All_familyInventoryIsActive()
  ...
  return 0 == InventoryInfo._familyInventoryIgnoreCount
end
...
function PaGlobal_FamilyInvnetory_GetToopTipItem()
  if nil == InventoryInfo._familyInventoryTooltipSlotNo then
    return nil
  end
  return getInventoryItemByType(WHERE_TYPE.FAMILY_INVENTORY, InventoryInfo._familyInventoryTooltipSlotNo)
end
