...
function PaGlobal_ServerSelect_All:initDesriptionData()
  ...
  if true == _ContentsGroup_ConquestSiege then
    warInfoDesc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_TITLE") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_DESC_BALENOS") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_DESC_SERENDIA") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_DESC_CALPEON") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_DESC_MEDIA") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_DESC_VALENCIA") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_TERRITORY_SUB_DESC_KAMA") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_DATE_SUB_TITLE") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_DATE_SUB_DESC_TERRITORYWAR") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_WARCHANNEL_DATE_SUB_DESC_NODEWAR")
  end
...
function PaGlobal_ServerSelect_All:initTreeData()
  ...
  for ii = 1, worldServerCount do
    ...
    self._restrictedServerData[ii] = {serverNo = restrictedServerNo, serverGroupNo = 0}
    local tempTable = {}
    ...
    for jj = 1, channelCount do
      ...
      if 0 ~= restrictedServerNo and restrictedServerNo == channelServerData._serverNo then
        self._restrictedServerData[ii].serverGroupNo = channelServerData._serverGroupNo
      end
...
function PaGlobal_ServerSelect_All:createChannelBranch(content, key)
  ...
  if restrictedServerNo ~= 0 then
    ...
    if 0 ~= self._restrictedServerData[worldServerIdx].serverGroupNo then
      local guildServerNo = ToClient_GetGuildServerGroupNo()
      if 0 ~= guildServerNo and channelServerData._serverGroupNo == self._restrictedServerData[worldServerIdx].serverGroupNo and channelServerData._serverGroupNo == guildServerNo then
        isAdmission = true
      end
    end
  end
  if true == channelServerData._isEventChannel then
    isAdmission = true
  end
  local admissionDesc = ""
  ...
  elseif channelServerData._isDontPvPTendencyDecrease then
    ...
    self:showChannelIcon(channelButtonBG, self._CHANNEL_TYPE.ARSHA_CHANNEL)
  elseif channelServerData._isEventChannel then
    self:showChannelIcon(channelButtonBG, self._CHANNEL_TYPE.OLYMPIC)
  end
  if true == channelServerData._isHadumChannel then
    self:showChannelIcon(channelButtonBG, self._CHANNEL_TYPE.HADUM)
  end
  local guildServerNo = ToClient_GetGuildServerGroupNo()
  if 0 ~= guildServerNo and channelServerData._serverGroupNo == guildServerNo then
...
function PaGlobal_ServerSelect_All:enterMemorizedChannel(channelIdx)
  ...
  if channelServerData._isSeasonNewbieChannel then
    ...
    if false == isAccessible then
      ...
      local newbieStr = ""
      if true == channelServerData._isSeasonChannel then
